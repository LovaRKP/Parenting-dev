...
@interface ReviewsViewViewController ()  <UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UIActionSheetDelegate>
{
    NSString *type;
...
- (void)viewDidLoad {
    ...
    self.screenName = @"Reviews Screen";
    ...
    [_myActivieIndicater startAnimating];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if(![[NSUserDefaults standardUserDefaults] boolForKey:@"LanguageSettings"]) {
        ...
        [self.navigationController.navigationBar.topItem setTitle:@"REVIEWS"];
        self.navigationController.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName: [UIColor whiteColor]};
        
    }else {
        
        [self.navigationController.navigationBar.topItem setTitle:@"समीक्षा"];
        self.navigationController.navigationBar.titleTextAttributes = @{NSForegroundColorAttributeName: [UIColor whiteColor]};
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ReviewsCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CellIdentifier forIndexPath:indexPath];
    ...
    cell.bodderdAge.layer.borderColor = [UIColor whiteColor].CGColor;
    cell.bodderdAge.layer.borderWidth = 1.0;
    cell.bodderdAge.backgroundColor = [UIColor colorWithRed:116.0f/255.0f
                                                      green:79.0f/255.0f
                                                       blue:141.0f/255.0f
                                                      alpha:1.0f];
    cell.bodderdAge.layer.cornerRadius = 8.0;
    
    cell.bodderdRating.layer.borderColor = [UIColor whiteColor].CGColor;
    cell.bodderdRating.layer.borderWidth = 1.0;
    cell.bodderdRating.layer.cornerRadius = 8.0;
    
    
    
    
    cell.title.text = [[myArray objectAtIndex:indexPath.row]objectForKey:@"title"];
    ...
    cell.discriptionReview.attributedText = attributedString;
    
    
    
    if (IS_IPAD)
    {
       cell.discriptionReview.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:18.0];
        
    }else{
        
    cell.discriptionReview.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:13.0];
        
    }
    cell.discriptionReview.textColor = [UIColor whiteColor];
    ...
    if ([emptyAge isEqualToString:@""]) {
        ...
        cell.ageLeabel.text = @"All";
        
    }else{
        cell.ageLeabel.text = [mydicvalue objectForKey:@"age_group"];
        
    }
    
    
    cell.reatingLeabel.text = [mydicvalue objectForKey:@"rating"];
    ...
    NSURLRequest *request = [NSURLRequest requestWithURL:[NSURL URLWithString:imageurl]
                                         ...
                                         timeoutInterval:60];
    [cell.reviewImage setImageWithURLRequest:request
    ...
    if ([reviewwType isEqualToString:@"book"]) {
        cell.TitleReview.text = @"Book Review";
        
    }else if ([reviewwType isEqualToString:@"movie"]){
        cell.TitleReview.text = @"Movie Review";
    
    
    }else if ([reviewwType isEqualToString:@"tv"]){
    
    cell.TitleReview.text = @"TV Review";
    
    
    }else{
        cell.TitleReview.text = @"Reviews";
    ...
    if([reviewwType isEqualToString:@"book"]) {
        ...
        cell.bodderdAge.hidden = YES;
        cell.reatingLeabel.hidden = YES;
        cell.rattingLeabelHidden.hidden = YES;
        cell.hiddenLeabel.hidden = YES;
        
        
    }else {
    
        cell.bodderdAge.hidden = NO;
        cell.reatingLeabel.hidden = NO;
        cell.rattingLeabelHidden.hidden = NO;
        cell.hiddenLeabel.hidden = NO;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    CGSize mElementSize  = CGSizeMake(300, 290);
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
    {
        ...
        {
            // iPhone 5
            CGSize mElementSize  = CGSizeMake(350, 290);
            return mElementSize;
            
        }
        else if(result.height == 736)
        {
            CGSize mElementSize  = CGSizeMake(380, 290);
            return mElementSize;
        }
    ...
    if (IS_IPAD)
    {
        CGSize mElementSize  = CGSizeMake(700, 403);
        return mElementSize;
